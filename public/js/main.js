...
function setActiveNav() {
  ...
  const pageBase = page.replace('.html', '').toLowerCase();
  ...
  const currentRoute = routeAliasMap[pageBase] || pageBase;

  document.querySelectorAll('.navbar-nav a').forEach(link => {
    const href = link.getAttribute('href') || '';
    const hrefBase = href.split('/').pop().replace('.html', '').toLowerCase();
    const linkRoute = routeAliasMap[hrefBase] || hrefBase;
...
function setupFooterLinks() {
  const infoMap = {
    'methodology & metrics': 'Methodology & Metrics: Formulated according to NSDC & MSDE 2026 data governance standards.',
    'open data standards': 'Open Data Standards: Integrated with National Skill Registry Schema v2.4.',
    'api documentation': 'API Documentation: REST & GraphQL endpoints available for authenticated state partners.',
    'quarterly reports': 'Quarterly Reports: Q3 FY25-26 National Skill & Outcome Intelligence Report is available.',
    'privacy policy': 'Privacy Policy: Trainee records are securely governed under the DPDP Act 2023.',
    'terms of service': 'Terms of Service: Standard Government of India portal terms and data policies apply.',
    'data governance': 'Data Governance: Role-based access control with multi-tier audit logging active.',
    'security compliance': 'Security Compliance: ISO 27001 certified and CERT-In audited infrastructure.',
  };

  document.querySelectorAll('.footer-links a').forEach(link => {
    const text = (link.textContent || '').trim().toLowerCase();
    const href = link.getAttribute('href');
    if (href === '#' && infoMap[text]) {
      link.addEventListener('click', (e) => {
        e.preventDefault();
        showToast(infoMap[text], 'info');
      });
    }
  });

  document.querySelectorAll('.developed-by-logo-wrap').forEach(link => {
    link.addEventListener('click', (e) => {
      e.preventDefault();
      showToast('SkillTrack360 — Built with Rashomind Technology Solutions', 'info');
    });
  });
}
